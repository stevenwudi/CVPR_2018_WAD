...
def im_detect_all(args, model, im, dataset, box_proposals=None, timers=None):
    ...
    if cfg.MODEL.MASK_ON and boxes.shape[0] > 0:
        ...
        cls_segms, output_row = segm_results(args, cls_boxes, masks, boxes, im.shape[0], im.shape[1], dataset)
...
def rle_encoding(x):
    ...
    rle = '|'.join('{} {}'.format(*pair) for pair in run_lengths)
    return rle + '|'
...
def rle_encoding_official(InstanceMap):
